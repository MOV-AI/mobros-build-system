...
import os
import mock
from mobros.commands.ros_install_build_deps.install_deps_executer import InstallBuildDependsExecuter
...
@mock.patch("mobros.commands.ros_install_runtime_deps.install_deps_executer.InstallRuntimeDependsExecuter.execute", return_value=None)
@mock.patch("mobros.utils.utilitary.translate_package_name", return_value="ros-noetic-mobros")
@mock.patch(    
...
class TestInstallBuildDepsExecuter(unittest.TestCase):
    ...
    def test_execute_happy_path(self, mock_get_pkg_installed_version, mock_inspect_package, mock_get_versions, mock_is_pkg_installed, mock_execute_cmd, mock_rosdep_translate, mock_mobros_install_execute):
        TEST_RESOURCE_PATH_VALID = os.path.join(
            os.getcwd(), "tests", "resources", "test_dependencies", "tree_simple_valid_deps"
        )
        argparse_args = argparse.Namespace(
            workspace=TEST_RESOURCE_PATH_VALID,
            simulate=True
        )
        ...
        expected_install_args = argparse.Namespace(
            y=True,
            pkg_list=['ros-noetic-mobros=1.2.0-3'], 
